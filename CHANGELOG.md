...
### Fixed
- Refactored duplicated C-string parsing loops in VFS and process system calls into a shared helper function (`read_user_string`) for improved safety and cleanliness.
...
### Added
- Dynamic user-space program break expansion system call (`sys_sbrk`).
- Safe pointer-based heap page unmapping and frame reclamation on program exit (`vmm::free_and_unmap_page`).
- Custom heap memory allocator (`malloc`, `free`, `calloc`, `realloc`) integrated into user-space `libkeira`.
- Page Fault / Exception crash isolation for Ring 3 processes, allowing task termination and returning to shell instead of triggering a kernel panic.

## [0.8.1] - 2026-07-16

### Fixed
- Fixed user program execution error when calling `run init` by adding dynamic path lookup, suffix matching, and alias resolution.
- Suppressed and resolved C linter warnings and notes in `make lint`.
